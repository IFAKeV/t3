...
def get_tickets_with_filters(
    team_id=None, status_filter="open", search_term=None, assigned_agent_id=None
):
    ...
    base_query = """
        SELECT t.TicketID, t.Title, t.Description, t.StatusID, t.PriorityID, t.TeamID,
               ...
               -- Alter des Tickets in Tagen berechnen
    ...
    if assigned_agent_id:
        # Nur Tickets auswählen, die dem Agenten zugewiesen sind
        conditions.append(
...
def get_related_tickets(ticket, limit=5):
    """Verwandte Tickets anhand Person, Einrichtung oder Standort finden"""
    related = []
    seen_ids = set()

    # Tickets derselben Person ermitteln
    if ticket.get("ContactEmployeeID"):
        rows = query_db(
            ...
            JOIN TicketStatus s ON t.StatusID = s.StatusID
            JOIN Teams team ON t.TeamID = team.TeamID
            WHERE t.ContactEmployeeID = ? AND t.TicketID != ? AND t.StatusID != 3
            ORDER BY t.CreatedAt DESC LIMIT ?
            """,
            (ticket["ContactEmployeeID"], ticket["TicketID"], limit),
        )
        for row in rows:
        ...
        rows = query_db(
            ...
            JOIN TicketStatus s ON t.StatusID = s.StatusID
            JOIN Teams team ON t.TeamID = team.TeamID
            WHERE lower(t.ContactName) = lower(?) AND t.TicketID != ? AND t.StatusID != 3
            ORDER BY t.CreatedAt DESC LIMIT ?
            """,
            (ticket["ContactName"], ticket["TicketID"], limit),
        )
        for row in rows:
            if row["TicketID"] not in seen_ids:
                related.append(row)
    ...
    if ticket.get("FacilityID"):
        rows = query_db(
            ...
            JOIN TicketStatus s ON t.StatusID = s.StatusID
            JOIN Teams team ON t.TeamID = team.TeamID
            WHERE t.FacilityID = ? AND t.TicketID != ? AND t.StatusID != 3
            ORDER BY t.CreatedAt DESC LIMIT ?
            """,
            (ticket["FacilityID"], ticket["TicketID"], limit),
        )
        for row in rows:
            if row["TicketID"] not in seen_ids:
                related.append(row)
    ...
    # Tickets gleicher Standort, wenn nicht durch Einrichtung abgedeckt
    if ticket.get("LocationID"):
        rows = query_db(
            ...
            JOIN TicketStatus s ON t.StatusID = s.StatusID
            JOIN Teams team ON t.TeamID = team.TeamID
            WHERE t.LocationID = ? AND t.TicketID != ? AND t.StatusID != 3
