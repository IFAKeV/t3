...
def dashboard():
    ...
    # Zeigt standardmäßig meine offenen Tickets
    assignment_filter = request.args.get("assign", "mine")
    ...
    # Tickets laden
    # Standardmäßig nur Tickets zeigen, die mir zugewiesen sind
    tickets = get_tickets_with_filters(
        team_id=team_id,
        status_filter=status_filter,
        search_term=search_term or None,
        assigned_agent_id=(
            g.current_agent["AgentID"] if assignment_filter == "mine" else None
        ),
    )
...
def new_ticket():
    ...
    if request.method == "POST":
        ...
        # Speichert, wie das Ticket gemeldet wurde (Mail, Telefon, etc.)
        created_via = request.form.get("created_via")
        ...
        ticket_id = insert_db(
            ...
            [
                title,
                description,
                # Ersteller des Tickets festhalten
...
def view_ticket(ticket_id):
    ...
    # Verwandte Tickets anhand mehrerer Kriterien
