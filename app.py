...
from flask import Flask, render_template, request, redirect, url_for, jsonify, g, flash, send_from_directory
...
from database import (
    ...
    get_all_teams, get_all_statuses, get_all_priorities,
    load_agents, get_agent_by_token,
    get_tickets_with_filters, get_ticket_by_id,
    get_status_by_id, get_priority_by_id
)
from addressbook import (
    search_employees, get_employee_details, format_contact_info,
    get_facility_info, get_location_info, get_department_info,
...
def login():
    ...
    if request.method == 'POST':
        ...
        if agent:
            ...
            response.set_cookie('agent_token', token, max_age=30*24*60*60)  # 30 Tage
            return response
    ...
    if request.method == 'POST':
        ...
        if assigned_agent_id:
            assigned_agent = query_db(
                ...
                (assigned_agent_id,), one=True
            )
            if assigned_agent:
                insert_db('TicketAssignees', [
